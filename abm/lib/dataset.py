...
def _filter_datasets(datasets, state=None, dtype=None, name=None, unique=False):
    ...
    result = datasets
    if state:
        result = [ds for ds in result if ds.get('state', 'unknown') == state]
    if dtype:
        result = [ds for ds in result if ds.get('history_content_type') == dtype]
    if name:
...
def do_list(context: Context, argv: list):
    ...
    parser.add_argument(
        '-t', '--tool', help='only show datasets generated by this tool'
    )
    parser.add_argument(
        '--type',
        help='only show datasets of this type (e.g. dataset, dataset_collection)',
    )
    parser.add_argument(
        '-n', '--name', help='only show datasets whose name matches this regex'
    )
    parser.add_argument(
        '-u',
        '--unique',
        action='store_true',
        help='ignore datasets with duplicate names',
    )
    ...
    datasets = _filter_datasets(
        datasets, dtype=args.type, name=args.name, unique=args.unique
    )
    if len(datasets) == 0:
...
def copy(context: Context, argv: list):
    parser = argparse.ArgumentParser()
    parser.add_argument('--source', required=True, help='source history name or ID')
    parser.add_argument(
        '--target',
        required=True,
        help='target history name or ID (created if it does not exist)',
    )
    parser.add_argument('-s', '--state', help='only copy datasets in this state')
    parser.add_argument(
        '--type',
        help='only copy datasets of this type (e.g. dataset, dataset_collection)',
    )
    parser.add_argument(
    ...
    datasets = gi.datasets.get_datasets(**kwargs)
    datasets = _filter_datasets(
        datasets, state=args.state, dtype=args.type, name=args.name, unique=args.unique
    )
